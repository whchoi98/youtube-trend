...
| Trends router | `backend/app/api/trends.py` | `GET /api/trends/categories?hours` (default 48, range 2-720): up to 96h it runs a full-bucket Query (safe inside the DynamoDB 1MB limit); beyond that it downsamples via `store.snapshots_sampled` at `step = ceil(hours/96)`-hour GetItem intervals (anchor = the `until` floor bucket, walking back 1h up to step-1 times when the anchor bucket is missing), so reads stay constant; the response carries `stepHours`, and `entered`/`exited` become changes between adjacent sampled buckets |
| Chart history router | `backend/app/api/charts.py` | `GET /api/charts/{chartId}/videos/{videoId}/history?hours` (default 168, range 2-720 — per-song points are small items, so a full month is allowed; the old snapshot-derived 72-hour cap and its rationale are gone) returning `{chartId, videoId, points}` from a `CVID#` point Query (`chart_video_history`) written by the collector; hours where the song is off the chart simply have no point, so the line breaks; history from before the transition is not shown — points accumulate only from post-deploy collections; unknown `chartId` -> 400 `{"error": "지원하지 않는 차트입니다"}`; no points -> 200 with empty `points` |
...
- The quiz is deterministic and LLM-free: vibe tag match +3, `MOOD_CATS`/`TIME_CATS`/`STYLE_CATS` category weights, rank tiebreak `(31-rank)/30`; answers are validated in-router against the fixed vocabularies and reuse the same 400 message as the global validation remap.
- `/api/trends/categories` keeps the null-vs-0 contract in its series: the first bucket's `entered`/`exited` are `null` (no previous bucket to diff against — `aggregate.category_series`); when downsampling is active (`stepHours > 1`) the diffs are step-interval changes, not hourly ones.
...
- `backend/app/home.py` — `build_rows`, `build_insights`, `tenure_hours`, `quiz_type`, `quiz_pick`
...
| 추이 라우터 | `backend/app/api/trends.py` | `GET /api/trends/categories?hours`(기본 48, 범위 2~720): 96h까지는 전 버킷 Query(DynamoDB 1MB 한도 내 안전 마진), 초과 시 `store.snapshots_sampled`로 `step = ceil(hours/96)`시간 간격 GetItem 다운샘플(앵커 = `until`의 floor 버킷, 앵커 결손 시 1시간씩 최대 step-1회 후퇴 탐색) — 읽기량이 기간과 무관하게 상수다. 응답에 `stepHours` 포함, `entered`/`exited`는 인접 샘플 버킷 간 변화량이 된다 |
| 차트 이력 라우터 | `backend/app/api/charts.py` | `GET /api/charts/{chartId}/videos/{videoId}/history?hours`(기본 168, 범위 2~720 — 곡별 포인트가 소형 아이템이라 한 달까지 허용. 스냅샷 파생 시절의 72 캡과 그 사유는 폐기) — 수집기가 적재한 `CVID#` 포인트 Query(`chart_video_history`)로 `{chartId, videoId, points}` 반환. 곡이 차트에 없던 시각은 포인트 자체가 없어 선이 끊긴다. 전환 시점 이전의 스냅샷 기반 이력은 표시되지 않는다 — 포인트는 배포 후 수집부터 축적된다. 잘못된 `chartId` -> 400 `{"error": "지원하지 않는 차트입니다"}`, 포인트 없으면 200 + 빈 `points` |
...
- 퀴즈 추천은 결정적이며 LLM을 호출하지 않는다: 태그 vibe 일치 +3, `MOOD_CATS`/`TIME_CATS`/`STYLE_CATS` 분야 가중치, 순위 타이브레이크 `(31-rank)/30`. 답변은 라우터에서 고정 어휘로 검증하며 전역 검증 재매핑과 동일한 400 메시지를 쓴다.
- `/api/trends/categories`의 시계열도 null vs 0 계약을 지킨다: 첫 버킷의 `entered`/`exited`는 `null`이다(비교할 이전 버킷 없음 — `aggregate.category_series`). 다운샘플 시(`stepHours > 1`) 변화량은 시간당이 아니라 step 간격 기준이다.
