...
| Chart series panel | `frontend/src/components/ChartSeriesPanel.tsx` | "YouTube Music 시계열" panel on the "시계열 추이" view — chart rows pulled from `/api/home` feed a chart + song selector (switching charts resets to that chart's #1 when the song is absent); history via `/api/charts/{chartId}/videos/{videoId}/history?hours=72`, rendered with `HistoryCharts maxRank={20}`; a `.music-link` "YouTube Music에서 듣기" link for the selected song; generation-guarded like the other panels |
...
| 차트 시계열 패널 | `frontend/src/components/ChartSeriesPanel.tsx` | "시계열 추이" 화면의 "YouTube Music 시계열" 패널 — `/api/home`의 chart 행 목록으로 차트+곡 셀렉터 구성(차트 전환 시 선택 곡이 없으면 그 차트 1위로 재설정). `/api/charts/{chartId}/videos/{videoId}/history?hours=72`로 조회해 `HistoryCharts maxRank={20}`으로 렌더. 선택 곡의 "YouTube Music에서 듣기" 링크(`.music-link`) 포함. 다른 패널과 동일한 세대 가드 |
