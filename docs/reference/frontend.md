...
| Contract types | `frontend/src/types.ts` | `Card`/`HomeCard`/`HomeRow`/`HomeHero`/`HomeData`/`QuizAnswers`/`QuizResult` plus `Category`/`HistoryPoint`/`TrendBucket`/`Loadable`/`ChannelStat` (nullable derived fields; optional `description` and `Tags.comment` — absent on snapshots/tag buckets stored before each field was introduced; `RowKind` includes `new`/`climb`/`chart`/`spotlight`/`vibe`/`region` (`age` removed), `HomeRow` carries optional `regionCode`/`chartId`; `HomeData.channels` is `ChannelStat[] \| null` with `subscribers: number \| null`) |
...
### 1. 개요
React 18 + Vite + TypeScript SPA다. 라우터는 없고(로고·페이지 타이틀은 "YOUTUBE TREND MONITOR"로 개명 — 구 TREND RADAR, 레이아웃 콘셉트는 그대로다), 톱바(로고, 상단 메뉴, 수집 시각 KST, 새로고침, 테마 버튼)의 기존 탭 스타일 상단 메뉴가 `view` 상태 하나로 3화면을 전환한다: "홈"(히어로 빌보드 — 기본 전체 1위, 타일 선택 시 그 콘텐츠 — + 선택 콘텐츠 추이 패널 + 인사이트 칩 + 주제 사이드바 옆 가로 스트립 행들 top10/accel/new/climb/chart(YouTube Music 5행)/topic/vibe/category/region/spotlight + 클라이언트 삽입 퀴즈 행 + 급상승 채널 아바타 스트립), "시계열 추이"(영상 시계열 + YouTube Music 시계열 패널), "점유율 · 리포트"(카테고리 점유율 추이 + AI 브리핑). 백엔드 `/api/*` 계약만 소비하며, 게이트는 `npx tsc --noEmit && npm run build`다(테스트 러너 없음).
...
| 히어로 | `frontend/src/components/Hero.tsx` | 넷플릭스식 빌보드 — 기본은 전체 1위, 타일 선택 시 그 카드의 제목·카테고리 칩(`.hero-cat`)·간단한 소개 `description`(`.hero-desc`)·YouTube 링크·"✕ 1위 화면으로" 해제 버튼으로 전환. 표시 중인 카드에 `tags.comment`가 있으면 AI 한 줄 분석(`.hero-ai` — "AI" `.ai-chip` 텍스트 칩 + `tags.comment`)을 렌더. maxres 배경 `img` + videoId 단위 `onError` 폴백, 차트인 시간, NEW 칩, 보러가기/내 취향 찾기 |
