...
| Hero | `frontend/src/components/Hero.tsx` | Netflix-style billboard — overall #1 by default; a tile-selected card swaps in its title, category chip (`.hero-cat`), short `description` synopsis (`.hero-desc`), YouTube link, and a "✕ 1위 화면으로" clear button; an AI one-liner (`.hero-ai` — an "AI" `.ai-chip` text chip plus `tags.comment`) renders whenever the shown card has `tags.comment`; maxres background `img` with per-videoId `onError` fallback, chart-in tenure hours, NEW chip, watch / quiz CTAs — the watch CTA routes by selection origin via the `music` prop: chart-origin selections get `musicUrl` + "▶ YouTube Music에서 듣기", everything else `youtubeUrl` + "▶ 보러가기" |
...
| History charts | `frontend/src/components/HistoryCharts.tsx` | Shared rank + views `LineChart` pair with a log/linear toggle and a `maxRank` prop (default 30 — rank Y-axis domain) — rendered by `SelectedTrend`, `VideoSeriesPanel`, and `ChartSeriesPanel` (replaces the deleted `DetailModal.tsx`) |
...
| Period toggle | `frontend/src/components/PeriodToggle.tsx` | Shared pill toggle for the two series panels (`.period-toggle`/`.period-btn`, `role="group"`) — `PERIODS` 24시간/일주일/한 달 = 24/168/720 hours |
...
| 시계열 차트 | `frontend/src/components/HistoryCharts.tsx` | 공용 순위+조회수 `LineChart` 쌍 + 로그/선형 토글 + `maxRank` prop(기본 30 — 순위 Y축 도메인) — `SelectedTrend`·`VideoSeriesPanel`·`ChartSeriesPanel`이 렌더(삭제된 `DetailModal.tsx` 대체) |
...
| 기간 토글 | `frontend/src/components/PeriodToggle.tsx` | 두 시계열 패널이 공유하는 알약 토글(`.period-toggle`/`.period-btn`, `role="group"`) — `PERIODS` 24시간/일주일/한 달 = 24/168/720시간 |
