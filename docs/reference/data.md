...
| Store access layer | `backend/app/store/table.py` | `TrendStore`: `put_snapshot` (conditional write), `latest_snapshot`, `baseline_snapshot` (offset fallback, never raises), `snapshots_range`, `put_video_points`, `video_history` (also serves `/api/home` hero tenure over a 72h window), `put_chart_points`/`chart_video_history` (per-chart-song points — small items that make month-long (720h) history a light Query instead of a snapshot-range scan at ~14KB/bucket; TTL 30 days), `get_report`/`put_report`, `get_tags`/`put_tags`; Decimal-to-int normalization |
