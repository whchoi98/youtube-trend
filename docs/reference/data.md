...
### 1. Overview
Single DynamoDB table (`pk`/`sk`) stores hourly trending snapshots, per-video and per-chart-song time series, LLM report caches, and AI tag results, all expired by TTL (snapshots and time-series points 30 days, reports 2 days, tags 2 days). Key rules live in exactly one module so every reader and writer computes identical keys.

### 2. Components
| Component | Path | Purpose |
|---|---|---|
| Key rules (single source) | `backend/app/store/keys.py` | `snap_pk` resolves scope strings by prefix: `all` → `SNAP#ALL`, `rgn-{CODE}` → `SNAP#RGN#{CODE}`, `spot-{name}` → `SNAP#SPOT#{name}`, `chart-{name}` → `SNAP#CHART#{name}`, `chan-{name}` → `SNAP#CHAN#{name}`, anything else → `SNAP#CAT#{id}`; `vid_pk` (`VID#{videoId}`), `chart_vid_pk` (`CVID#{chart}#{videoId}`), `report_pk` (`REPORT#{kind}#{scope}`), `tags_pk` (`TAGS#ALL`), `ts_sk` (`TS#YYYY-MM-DDTHH`), `hour_bucket`, `ttl_epoch`; fallback constants `RECENT_OFFSETS`/`DAILY_OFFSETS`/`MIN_AGE_HOURS` |
| Store access layer | `backend/app/store/table.py` | `TrendStore`: `put_snapshot` (conditional write), `latest_snapshot`, `baseline_snapshot` (offset fallback, never raises), `snapshots_range`, `snapshots_sampled` (step-interval GetItem downsampling for ranges beyond 96h — reads stay constant at <= 96 buckets plus <= step-1 anchor probes; anchor = the `until` floor bucket, walking back 1h when it is missing), `put_video_points`, `video_history` (also serves `/api/home` hero tenure over a 72h window), `put_chart_points`/`chart_video_history` (per-chart-song points — small items that make month-long (720h) history a light Query instead of a snapshot-range scan at ~14KB/bucket; TTL 30 days), `get_report`/`put_report`, `get_tags`/`put_tags`; Decimal-to-int normalization |
...
| Spotlight catalog | `backend/app/spotlights.py` | `SPOTLIGHTS` single definition — (scope suffix, channel handle, row title "{name} 인기 영상") 3 entries: aws(AWSKorea)/anthropic(anthropic-ai)/openai(OpenAI); shown at the bottom of home in definition order; 3 quota units per channel per hour (uploads-playlist id cached per instance); replaces the old `SPOTLIGHT_HANDLE`/`SPOTLIGHT_SCOPE` constants in `run.py` |
...
- TTL attribute `expireAt`: snapshots and chart-song points 30 days (`SNAPSHOT_TTL_DAYS`), LLM reports 2 days, tags 2 days (`TAGS_TTL_DAYS`).
...
### 1. 개요
DynamoDB 단일 테이블(`pk`/`sk`)에 시간별 급상승 스냅샷, 영상별·차트 곡별 시계열, LLM 리포트 캐시, AI 태그 결과를 저장하고 TTL로 만료한다(스냅샷·시계열 포인트 30일, 리포트 2일, 태그 2일). 키 규칙은 단일 모듈에만 정의해 모든 읽기·쓰기 경로가 동일한 키를 계산한다.

### 2. 구성요소
| 구성요소 | 경로 | 목적 |
|---|---|---|
| 키 규칙(단일 정의) | `backend/app/store/keys.py` | `snap_pk`가 스코프 문자열을 접두로 해석한다: `all` → `SNAP#ALL`, `rgn-{CODE}` → `SNAP#RGN#{CODE}`, `spot-{name}` → `SNAP#SPOT#{name}`, `chart-{name}` → `SNAP#CHART#{name}`, `chan-{name}` → `SNAP#CHAN#{name}`, 그 외 → `SNAP#CAT#{id}`. `vid_pk`(`VID#{videoId}`), `chart_vid_pk`(`CVID#{chart}#{videoId}`), `report_pk`(`REPORT#{kind}#{scope}`), `tags_pk`(`TAGS#ALL`), `ts_sk`(`TS#YYYY-MM-DDTHH`), `hour_bucket`, `ttl_epoch`와 폴백 상수 `RECENT_OFFSETS`/`DAILY_OFFSETS`/`MIN_AGE_HOURS` |
| 저장소 접근 계층 | `backend/app/store/table.py` | `TrendStore`: `put_snapshot`(조건부 쓰기), `latest_snapshot`, `baseline_snapshot`(오프셋 폴백, 예외 없음), `snapshots_range`, `snapshots_sampled`(96h 초과 범위의 step 간격 GetItem 다운샘플 — 읽기량이 96버킷 이하 + 앵커 탐색 step-1회 이하로 상수. 앵커 = `until`의 floor 버킷, 결손 시 1시간씩 후퇴), `put_video_points`, `video_history`(`/api/home` 히어로 차트인 시간 계산에도 사용 — 72h 윈도), `put_chart_points`/`chart_video_history`(차트 곡별 포인트 — 소형 아이템이라 한 달(720h) 추이를 버킷당 ~14KB 스냅샷 범위 조회 대신 가벼운 Query로 처리. TTL 30일), `get_report`/`put_report`, `get_tags`/`put_tags`. Decimal→int 정규화 포함 |
...
| 스포트라이트 목록 | `backend/app/spotlights.py` | `SPOTLIGHTS` 단일 정의 — (scope 접미사, 채널 핸들, 행 제목 "{이름} 인기 영상") 3개 항목: aws(AWSKorea)/anthropic(anthropic-ai)/openai(OpenAI). 홈 맨 하단에 정의 순서로 표시, 채널당 쿼터 3유닛/시간(uploads 재생목록 id 인스턴스 캐시). `run.py`의 구 `SPOTLIGHT_HANDLE`/`SPOTLIGHT_SCOPE` 상수를 대체한다 |
...
- TTL 속성은 `expireAt`이다 — 스냅샷·차트 곡 포인트 30일(`SNAPSHOT_TTL_DAYS`), LLM 리포트 2일, 태그 2일(`TAGS_TTL_DAYS`).
