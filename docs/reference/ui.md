...
1. **Topbar** — "YOUTUBE TREND MONITOR" gradient logo, top menu (three buttons "홈"/"시계열 추이"/"점유율 · 리포트" in a rounded segment box — `.nav-tabs` 999px pill container, active tab `background: var(--accent)` + `color: var(--on-cta)`; `role="tablist"`, `VIEWS`/`view` state in `App.tsx`, no router; switching scrolls to top), capture time (`수집 HH:MM` KST via `formatClockKst`), refresh button, theme button (`App.tsx`)
2. **Hero billboard** — overall #1 video by default: `maxresdefault` background image with `onError` fallback to the stored thumbnail, chart-in tenure (`N시간째 차트인`), first-entry chip (`오늘 첫 진입`), watch link and quiz launcher; clicking any tile swaps in that card Netflix-billboard style — title, category chip (`.hero-cat`), short `description` synopsis (`.hero-desc`), YouTube link, and a "홈 화면으로" clear button — and the page scrolls to top; when the shown card has `tags.comment`, an AI one-liner (`.hero-ai`, prefixed by an "AI" `.ai-chip` text chip) appears (`Hero.tsx`)
...
| Period toggle | `frontend/src/components/PeriodToggle.tsx` | Pill toggle shared by the series panels and `TrendsPanel` (`.period-toggle` capsule, `.period-btn.active` accent fill) — default `PERIODS` 24시간/일주일/한 달 = 24/168/720 hours, overridable via the `periods` prop (TrendsPanel uses 48/168/720) |
...
| Trends panel | `frontend/src/components/TrendsPanel.tsx` | Category share stacked AreaChart + entered/exited BarChart with a `PeriodToggle` (48시간/일주일/한 달 via the custom `periods` prop, default 48h); when `stepHours > 1` a "N시간 간격 샘플" note appears; categories and trend loads live in separate effects so a period switch neither re-requests categories nor flashes the whole panel |
...
- `TrendsPanel` — stacked share AreaChart over `/api/trends/categories?hours={48|168|720}` plus an entered/exited BarChart (the first bucket's `entered`/`exited` are null, so it starts at the second bucket)
- `HistoryCharts` — rank LineChart (Y reversed, domain 1–`maxRank`, default 30) and views LineChart with a log/linear toggle; rendered in three places — `SelectedTrend` (under the hero, fixed 168h) and `VideoSeriesPanel` (`PeriodToggle`-selected hours), both over `/api/videos/{id}/history` via `useVideoHistory`, and `ChartSeriesPanel` (over `/api/charts/{chartId}/videos/{videoId}/history`, `PeriodToggle` default 168h, `maxRank={20}`; off-chart hours have no points, so the line breaks). In `windowHours` mode the X axis is a numeric time scale pinned to [now-period, now] (`bucketMs`/`formatMsKst`, 6-split ticks, M/D tick labels beyond 48h), gaps over 2h get null breakers so the line breaks instead of interpolating, and a `.coverage-note` reads "적재 N시간분 (X ~ Y)" — N is the actual point count — plus a per-panel coverage suffix ("급상승 목록/차트에 오른 동안에만 시간별로 적재됩니다")
...
- The top menu is a rounded segment box without a router: `.nav-tabs` is a 999px pill container and the active tab fills with `var(--accent)`; its foreground token is `--on-cta`, not `--on-accent` — the all-theme `#000` of `--on-accent` fails AA (4.46:1) on the neon-hunter accent. One `view` state switches the three views, every switch scrolls to top, and sub views sit in a `.page` container (renamed from `.bottom`); the 60s home polling runs regardless of the active view.
...
1. **톱바** — "YOUTUBE TREND MONITOR" 그라디언트 로고, 상단 메뉴(버튼 3개 "홈"/"시계열 추이"/"점유율 · 리포트"를 담은 라운드 세그먼트 박스 — `.nav-tabs` 999px 알약 컨테이너, 활성 탭은 `background: var(--accent)` + `color: var(--on-cta)`. `role="tablist"`, `App.tsx`의 `VIEWS`/`view` 상태, 라우터 없음. 전환 시 최상단 스크롤), 수집 시각(`수집 HH:MM` KST, `formatClockKst`), 새로고침 버튼, 테마 버튼 (`App.tsx`)
2. **히어로 빌보드** — 기본은 전체 1위 영상: `maxresdefault` 배경 이미지 + `onError` 시 저장된 썸네일 폴백, 차트인 시간(`N시간째 차트인`), 첫 진입 칩(`오늘 첫 진입`), 보러가기·퀴즈 버튼. 타일을 클릭하면 넷플릭스 빌보드처럼 그 카드로 전환된다 — 제목, 카테고리 칩(`.hero-cat`), 간단한 소개 `description`(`.hero-desc`), YouTube 링크, "홈 화면으로" 해제 버튼 — 그리고 페이지가 최상단으로 스크롤된다. 표시 중인 카드에 `tags.comment`가 있으면 AI 한 줄 분석(`.hero-ai` — "AI" `.ai-chip` 텍스트 칩이 앞에 붙는다)이 표시된다 (`Hero.tsx`)
...
| 기간 토글 | `frontend/src/components/PeriodToggle.tsx` | 시계열 패널들과 `TrendsPanel`이 공유하는 알약 토글(`.period-toggle` 캡슐, `.period-btn.active`는 액센트 채움) — 기본 `PERIODS` 24시간/일주일/한 달 = 24/168/720시간, `periods` prop으로 교체 가능(TrendsPanel은 48/168/720) |
...
| 추이 패널 | `frontend/src/components/TrendsPanel.tsx` | 카테고리 점유율 스택 AreaChart + 진입/이탈 BarChart — `PeriodToggle`(48시간/일주일/한 달, 커스텀 `periods` prop, 기본 48h). `stepHours > 1`이면 "N시간 간격 샘플" 표기. categories와 trend 로드 effect를 분리해 기간 토글 시 카테고리 재요청·패널 전체 플래시가 없다 |
...
- `TrendsPanel` — `/api/trends/categories?hours={48|168|720}` 기반 점유율 스택 AreaChart + 진입/이탈 BarChart(첫 버킷의 `entered`/`exited`는 null이라 둘째 버킷부터 그려진다)
- `HistoryCharts` — 순위 LineChart(Y 반전, 도메인 1–`maxRank`, 기본 30)와 조회수 LineChart(로그/선형 토글). 렌더 위치는 세 곳 — `SelectedTrend`(히어로 아래, 168h 고정)와 `VideoSeriesPanel`(`PeriodToggle` 선택 시간, 둘 다 `useVideoHistory` 경유 `/api/videos/{id}/history`), `ChartSeriesPanel`(`/api/charts/{chartId}/videos/{videoId}/history`, `PeriodToggle` 기본 168h, `maxRank={20}` — 차트 밖 시각은 포인트 자체가 없어 선이 끊김). `windowHours` 모드에서는 X축이 [지금-기간, 지금]에 고정된 숫자 시간축이 된다(`bucketMs`/`formatMsKst`, 6등분 ticks, 48h 초과 시 M/D 눈금). 2시간 초과 공백에는 null 브레이커를 넣어 선을 보간 대신 끊고, `.coverage-note`가 "적재 N시간분 (X ~ Y)"를 표기한다 — N은 실제 포인트 수이고, 패널별 적재 조건 문구("급상승 목록/차트에 오른 동안에만 시간별로 적재됩니다")가 덧붙는다
...
- 상단 메뉴는 라우터 없는 라운드 세그먼트 박스다: `.nav-tabs`가 999px 알약 컨테이너이고 활성 탭을 `var(--accent)`로 채운다. 전경 토큰은 `--on-accent`가 아니라 `--on-cta`다 — 전 테마 `#000`인 `--on-accent`는 neon-hunter accent 위에서 4.46:1로 AA 미달이다. `view` 상태 하나가 3화면을 전환하고, 전환 시마다 최상단으로 스크롤하며, 서브 화면은 `.page` 컨테이너(구 `.bottom`에서 개명)에 놓인다. 60초 홈 폴링은 활성 화면과 무관하게 돈다.
...
- 타이포그래피는 자체 호스팅 나눔스퀘어 웹폰트다: `src/assets/fonts/`의 woff2 3종을 가중치 400/500-700/800-900에 매핑하고 `font-display: swap`(시스템 폰트 선표시 후 교체)을 쓴다. Vite가 해시 자산으로 번들해 CloudFront/ALB 경계 안에서 외부 폰트 CDN 없이 서빙된다. 네이버 나눔글꼴 라이선스는 웹 임베딩을 허용한다. 폰트에 없는 글리프는 라벨에서 뺀다 — 히어로 해제 버튼은 ✕ 기호를 제거하고 "홈 화면으로"로 표기한다.
