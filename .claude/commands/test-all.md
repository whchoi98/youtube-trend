---
description: 전체 검증 실행 — backend pytest(129) → frontend tsc+build → infra synth → run-all.sh(있으면) → 라이브 스모크(배포 시)
...
- **판정 기준**: `129 passed`, failed/error 0. 수집 개수가 129 미만이면 통과처럼 보여도 수집 오류(import 실패 등)를 의심하고 FAIL로 다룬다.
